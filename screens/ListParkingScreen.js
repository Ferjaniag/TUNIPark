...
const ListParkingScreen = () => {
  const route=useRoute();
  const  duration=route.params.duree ;
  const date = route.params.datee ;
...
/*   locationss.map(data => {
   ...
   useEffect(() => {
    console.log("date",route.params.datee)
    firebase.firestore()
      .collection('locations')
      .get()
      .then((querySnapshot) => {
        setParkingId(querySnapshot.id) 
...
<LottieView   source={require("../assets/parking.json")} autoPlay loop
resizeMode="stretch"
/>
</View>
...
color:"#808080"
  ...
  export default ListParkingScreen;